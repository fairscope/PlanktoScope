...
def run_hierarchical(image_path, params):
...
def run_planktoscope_original(image_path, params):
...
def run_opencv_simple(image_path, params):
...
def run_watershed(image_path, params):
...
def main():
    try:
        ...
        sys.path.insert(0, "/home/pi/PlanktoScope/software/classifier")
        ...
        acq_info = get_acquisition_info(image_path)
        stats = load_live_stats(acq_info.get("acq_folder", ""))
        ...
        if method == "hierarchical":
            seg_result = run_hierarchical(image_path, params)
        elif method == "planktoscope":
            seg_result = run_planktoscope_original(image_path, params)
        elif method == "opencv":
            seg_result = run_opencv_simple(image_path, params)
        elif method == "watershed":
            seg_result = run_watershed(image_path, params)
        elif method == "auto":
            # Try methods in order of preference
            for fn in [run_hierarchical, run_planktoscope_original, run_opencv_simple]:
                seg_result = fn(image_path, params)
        ...
        if seg_result is None or seg_result.get("status") != "success":
            for fallback in [run_planktoscope_original, run_opencv_simple]:
                seg_result = fallback(image_path, params)
                if seg_result.get("status") == "success":
                    break
        ...
        # 7. Setup TSV file
        tsv_filename = f"ecotaxa_{acq_info['acq_id']}.tsv"
        ...
        # 9. Process and save each object
        saved_count = 0
        ...
        # 11. Save stats and publish MQTT
