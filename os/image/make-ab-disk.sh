...
sgdisk --new=5:0:+12G --typecode=5:8300 --change-name=5:"ROOTFS B" "${device}"

# Partition 6: Remaining space EXT4 "DATA"
sgdisk --new=6:0:0 --typecode=6:8300 --change-name=6:"DATA" "${device}"
...
mp1=$(mktemp -d)
mount "${device}1" "$mp1"
cp autoboot.txt "$mp1/autoboot.txt"
umount "${device}1"
