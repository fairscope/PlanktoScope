...
/**
 * Create calibration.json from defaults if it doesn't already exist.
 * Unlike initConfigFiles, this never overwrites — preserving user calibrations.
 */
export async function initCalibrationFile() {
  if (await hasConfig(CALIBRATION_PATH)) {
    return
  }
  await copyFile(CALIBRATION_DEFAULTS_PATH, CALIBRATION_PATH)
}
