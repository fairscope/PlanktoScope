...
export const installer = await service.getInterface(
  "/",
...
export async function getBundleInfo(path) {
  const raw = await installer.InspectBundle(path, [])
...
const url = "http://localhost/api/files/foo.raucb"
