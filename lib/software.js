...
export async function getGitSoftwareVersion() {
...
const update_url = "https://backend.eplankton.org/api/planktoscope-update"
// const update_url = "http://localhost:4000/rauc"
export async function checkForUpdate() {
  ...
  try {
    const [compatible, current_bundle_version] = await Promise.all([
      readProperty(installer, "Compatible"),
      getBootedSlotBundleVersion(),
    ])

    const url = new URL(update_url)
    url.searchParams.set("rauc_compatible", compatible)
    url.searchParams.set("rauc_version", current_bundle_version)

    const bundle_info = await getBundleInfo(url.toString())
    if (bundle_info.compatible !== compatible) return value
    if (bundle_info.version === current_bundle_version) return value
    value = bundle_info
  } catch (err) {
    console.error(err)
  }
